...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        h = F.relu(h)
        h = self.linear2(h)
        if self.use_sigmoid: # 마지막계층에서 시그모이드 함수 사용?
            h = F.sigmoid(h)
        return h
...
class VAE_2Tier(nn.Module): # 2계층 VAE
    def __init__(self, input_dim, hidden_dim, latent_dim):
        super().__init__()
    ...
    def get_loss(self, x):
        ...
        return (L1 + L2 + L3) / batch_size # 배치사이즈별 평균
...
if __name__ == "__main__":
    ...
    model = VAE_2Tier(input_dim, hidden_dim, latent_dim)
